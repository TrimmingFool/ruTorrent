...
//			}
			...
			if($req->run())
			{
				...
				if($req->success())
				{
					...
					if(User::isLocalMode())
					{
						...
						{
							$this->started = @filemtime(FileUtil::addslash($this->session).'rtorrent.lock');
	...
	public function readOpenFilesOfProcess()
	{
		$fdFile = FileUtil::addslash($this->session)."rtorrent.fds";
		if (User::isLocalMode() && !empty($this->session) && file_exists($fdFile)) {
			// Note: this file is not generated by rtorrent
			// Setup: Root executes 'ls -l /proc/$RTORRENT_PID/fd | wc -l'
			// and writes to a file which is readable by this apache user 
			$f = fopen($fdFile, "r");
			$fds = trim(fgets($f));
			fclose($f);
			return $fds;
